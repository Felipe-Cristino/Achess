...
const createRoomBtn = document.getElementById("funny-mode")
    ...
    rooms.forEach(room => {
        ...
        gamesList.innerHTML += `
            <li class='game' id='${room.id}'>
                ...
                <div>Funny Mode | ${room.id} | ${room.time}</div>
            </li>
        `
    })
}
...
socket.on("room-created", (id) => {
    ...
    window.location.href = window.location.origin + "/room3?id=" + id;
})

socket.on("room-joined", (id) => {
    
    window.location.href = window.location.origin + "/room4?id=" + id;
})
